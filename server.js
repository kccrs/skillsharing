...
let respond = (response, status, data, type) => {
...
let respondJSON = (response, status, data) => {
...
let readStreamAsJSON = (stream, callback) => {
  let data = '';
...
let waitForChanges = (since, response) => {
...
let getChangedTalks = (since) => {
  let found = [];
  let alreadySeen = (title) => {
    return found.some((f) => {
